...
_header = "[util.data_processing] "
def dprint(*args, **kwargs):
    return print(_header, *args, **kwargs)
...
class GifReader(object):
    ...
    def save_each_frame(self, path, savedir):
        '''
        Iterate the GIF, extracting each frame.
        '''
        mode = self._analyseImage(path)['mode']

        im = Image.open(path)

        i = 0 # start frame indexing at 0
        p = im.getpalette()
        ...
        try:
            while True:
                ...
                frame_save_name = '%s-%03d.png' % ('_'.join(os.path.basename(path).split('.')[:-1]), i)
                new_frame.save(os.path.join(savedir, frame_save_name), 'PNG')
